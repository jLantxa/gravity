...
void Universe::addParticle(ParticleType type,  float mass, float r, float x, float y, float vx, float vy) {
    Particle particle;
    particle.type = type;
    particle.mass = mass;
    particle.r = r;
    ...
    LOGI("Add particle: mass=%f, x=%f, y=%f, vx=%f, vy=%f\n",
        particle.mass, particle.x, particle.y, particle.vx, particle.vy);
}

void Universe::addStar(float x, float y) {
    addParticle(PARTICLE_STAR, BLACK_HOLE_MASS, BLACK_HOLE_R, x, y, 0, 0);
}

void Universe::addPlanet(float x, float y, float vx, float vy) {
    addParticle(PARTICLE_PLANET, PLANET_MASS, PLANET_R, x, y, vx, vy);
}
